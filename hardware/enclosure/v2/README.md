...
> **v2, in progress.** The base is now a dock: a TinyS3 runs the four sensors, and a USB-C socket at the back
> powers the dock and, through the pogo connector, the head ([DOCK.md](../../../docs/DOCK.md)). The Fusion design
> is **Env Monitor Enclosure v2**. [Wiring the TinyS3](#wiring-the-tinys3) describes v2. The other sections still
> describe v1, and the files in `stl/`, `step/` and `3mf/` are still v1's. Both are updated when the dock's
> layout is final.
...
- **2026-09-16**, v2:
  ...
  - The Inkplate sits as it comes again: the USB-C, the power button and the microSD are in the right wall, and the
    wake button is in the left wall. v1 turned the board 180° to put its USB-C, then the power input, on the left.
    The dock now powers the head through the pogo connector. The thick wall moved to the right, and the head moved
    5.3 mm left in its own frame, so the display, the bezel and the pogo connector are where they were on the dock.
    The back-cover grille moved up to the ESP32, now at the top of the board. The firmware's `kRotation` is 0.
...
- **2026-09-17**, v2 wiring:
  ...
  - The chain has one ground return, cable 1's GND conductor, and no second wire to the SCD41. The whole chain
    ...
    of ground shift, which I2C does not notice. v1's second return covered seven loose wires over the pogo joint,
    which v2 no longer has.
