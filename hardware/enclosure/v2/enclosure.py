...
#   head_matrix() moves the head that far left to sit centred on the base. The head's two wires come from the VIN
#   and GND pads on the top edge (see the head wiring), 70 mm from the connector.
POGO_X = (HEAD_X0 - HEAD_LWALL + HEAD_X1 + HEAD_RWALL) / 2   # 67.95 in the head, 62.65 in the base
POGO_Z = -6.00
...
# the top skin is flat at H 30.5 to D 37.7, then slopes to H 25 at the rear. PM sits on the LEFT. The base is a dock:
...
def head_matrix(tilt_deg=TILT, front_d=HEAD_FRONT_D, front_h=HEAD_FRONT_H):
    ...
    dx = (B_X0 + B_X1) / 2 - POGO_X                                                          # centres the head on the base
...
#   edge) and ts_box() puts it in the base: lying across behind the SCD41 compartment, the USB-C at the right end and
...
def ts_box(x0, x1, y0, y1, h0, h1):
    """A box given in the TinyS3's frame, placed in the base: x runs left from the USB-C end, y forward from J4's edge."""
...
TILE_D, TILE_T = 7.8, 3.2                     # LEGO 1x1 round tile (35380 / 98138), standard sizes, not yet measured
...
def build_pw_holder(body):
    """The USB-C power socket's seat: a pad under the board, two under the ears either side of the receptacle, side
    guides, and two corner stops at the pad edge, which leave the pads' middle free for the wires. Glued."""
    fl, h0 = 2.0 - 0.01, PW_H - PW_T / 2
    b0, b1 = PW_MOUTH_D - PW_L, PW_MOUTH_D
    ch = B_DBAY1 - CH_INSET
    wg = PW_W / 2 + 0.3                                                                      # the guides' inner faces; the pads
    union(body, boxb(PW_X - wg, PW_X + wg, b0 - 0.3, b1 - PW_REC_L - 0.25, fl, h0))         # run out to them and to the stops,
    for s in (-1, 1):                                                                        # so no 0.3 slot is left under the board
        xe0, xe1 = sorted((PW_X + s * (PW_REC_W / 2 + 0.45), PW_X + s * wg))
        union(body, boxb(xe0, xe1, b0 - 0.3, ch, fl, h0))                                    # under the ears
...
def build_chassis(basec, mh):
    ...
    px = bore_back.x / M                                                                     # the connector's X in the base
...
def build_pw_socket(elecc):
    """The 24-pin USB-C module as a block-out: board with the receptacle's notch, receptacle, and the pad row."""
    b0, b1 = PW_MOUTH_D - PW_L, PW_MOUTH_D
    ...
    pads = None
    for xc, w, lab in PW_PADS_TOP:
        p = boxb(xc - w / 2, xc + w / 2, PW_PAD_D0, PW_PAD_D1, h1, h1 + 0.05)
...
# --- base wiring (base frame X, D, H): the dock's wires, DOCK.md section 5 ---
#   The USB-C socket is the star point: three VBUS wires and three GND wires leave its pad row, to the TinyS3, the
#   AMS1117 and the pogo female. The TinyS3's cradle channel, open at both ends under the board, is the dock's main
...
#   is cable 1's GND conductor: under 300 mA at worst on 28 AWG, a few millivolts, so v1's second return is gone. Wires that travel together are
...
def build_base_wiring(basec, mh):
    ...
    def add_bundle(name_cols, centre, offsets, e1, tails, heads, d, want=None, lead=(0.0, 0.0), joint_ends=(True, True), r_first_tail=None):
        """A bundle along base-frame centreline centre; member k (name, colour) is offset offsets[k] across e1, runs
    ...
    #      over the PM group), up to H_POGO_CH, the length of the channel, over the holder's guide, and onto pads 9
    #      and 12 from behind. Bare bridges join the second tail of each net. ----
    ...
    # ---- the status LED: IO6 (J4 pin 8) through a 1 k resistor to the upper leg, GND to the socket's pad 1 from the
    #      lower one (one wire per joint: the strip's GND leg keeps cable 1's wire alone). A bundle from the pit:
    #      rearward over the SHTC3's right end, left of the shell pillar at (125, 30) onto the ribbon lane at X 126 -
    #      over the ribbon, under the baffle notch's roof - up to H_LED_CH past the BME688, and across the bay behind
    #      it. At X 86 they part: IO6 turns into the channel's right end at H_LED_CH and drops onto its leg; GND comes
    #      down to 13.5, runs along D 83.4 and ends on the GND splice's right end. ----
    t1 = LED_TIP + LED_L
    def leg(up):
        p = led_at(t1 + 2.0, up); return (p.x / M, p.y / M, p.z / M)                # Fusion (x, y, z) = base (X, D, H)
    hi, lo = leg(1.27), leg(-1.27)
    X_IO6, X_LEDG = 117.45, 118.95
    RES_D0, RES_D1, RES_R = 15.8, 21.8, 1.15                                        # the resistor's body, in the pit
    bodies.append(('resistor 1k', wire_b([(X_IO6, RES_D0, hi[2]), (X_IO6, RES_D1, hi[2])], 2 * RES_R, name='resistor'), 'beige'))
    add_bundle([('wire IO6: TinyS3 J4.8 -> LED (via 1k)', 'yellow'), ('wire GND: splice -> LED', 'black')],
               [(118.2, 26.0, 6.4), (118.2, 34.0, 10.0), (118.2, 41.0, 10.0), (125.3, 44.5, 9.0), (125.3, 53.0, 9.0), (125.3, 66.0, H_LED_CH),
                (125.3, 78.1, H_LED_CH), (86.0, 78.1, H_LED_CH)],
               [-0.75, 0.75], (1.0, 0.0, 0.0),
               [[(80.8, 77.35, H_LED_CH), (80.8, 90.4, H_LED_CH), (xl['IO6'], 90.4, H_LED_CH), (xl['IO6'], 90.4, 11.0), (xl['IO6'], 90.4, H_CH_LOW),
...
def run(context):
    ...
    board, builds the four printed parts (and the wiring layers, with BUILD_WIRING), then tilts the head into the base."""
    ...
    if BUILD_WIRING:                                # toggle these two components' light bulbs to hide the wiring
        wiring = [build_head_wiring(head.component), build_base_wiring(elec, mh)]
    else:
        wiring = []
        for grp in (head.component, base.component, elec):
    ...
    dock = [build_ts_strips(elec), build_pw_socket(elec), build_plug(elec), build_status_led(elec), build_led_tile(elec)]
    refs = build_pogo_ref(root)
    colour_all(des, app, head.component, elec, wiring + [p1, p2] + dock + refs)
    fin = {}
