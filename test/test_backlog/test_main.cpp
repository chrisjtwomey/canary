// The readings the board could not post: held in memory or on a card, and
// sent later, oldest first.
#include <unity.h>
...
void test_drain_sends_the_oldest_few_per_pass() {
    ...
    TEST_ASSERT_EQUAL_UINT32(5, drainBacklog(ring, server, 5, buf, sizeof(buf)));
...
void test_drain_stops_at_the_first_that_must_wait_and_keeps_it() {
    ...
    TEST_ASSERT_EQUAL_UINT32(1, drainBacklog(ring, server, 5, buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_UINT32(3, ring.count());
...
void test_drain_drops_a_document_the_server_refuses() {
    ...
    TEST_ASSERT_EQUAL_UINT32(3, drainBacklog(ring, server, 5, buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_UINT32(0, ring.count());
